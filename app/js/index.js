$(document).ready(function () {
    var companyId = getCompanyId('__biz'); //线上企业Id;
    localStorage.companyId = companyId;     //存储企业id
    $('#back').click(function () {
        window.history.back();
    });

    var code = getCompanyId('code');
    localStorage.code = code;

    if (code != '') {
        $.ajax({
            'url': "http://live.v114.com/mediacloud/mediadefault/test",
            'type': 'post',
            'data': JSON.stringify({'code': code, 'company_id': companyId}),
                ...
                if (data.state == 0) {
                    ...
                    $.each(data, function (i, ele) {
                        str += '<div class="swiper-slide">' +
                            '<a href="./details.html?qy_companyid=' + localStorage.companyId + '&cid=' + ele.cid + '">' +
                ...
                success: function (data) {
                    if (data.state.rc == 0) {
                        ...
                        $.each(result, function (i, ele) {
                            var commentList = {
                                nameSearch: ele.name,
                                nameLink: nameLink+ele.cid+"&qy_companyid="+localStorage.companyId
                            };
                            var chatHtml;
