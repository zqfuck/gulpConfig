...
var appTwo=new Vue({
	...
	methods:{
				...
				success:function(data){
					if(data.state.rc>=0){
						$.ajax({
							...
							dataType:"json",
							data:{
								id:data.detail_data.mediacloud_id,
								type:4
							},
							success:function(data){
								console.log(data);
						...
						$('body').append("<script> $(function(){console.log(localStorage.qnTid);function() {  onlineNum(localStorage.qnTid,'callback',localStorage.cu_user_name); } });</script>");
						...
						//分享
						var title = data.detail_data.name;
						var desc =data.detail_data.content_desc;
						var imgUrl=data.live_data.content_img;
						shareToOther(title,desc,imgUrl);
		...
		toRelated:function(cid){
			window.location.href="./details.html?cid="+cid+"&qy_companyid="+localStorage.companyId;
		},
		guest:function(){
			var that=this;
			$.ajax({
				url:URL+"/webapi/banner/guests",
				type:'get',
				dataType:"json",
				data:{
					cid:this.cid
				...
				success:function(data){
					console.log(data);
					if(data.state>=0){
						that.guests=data.data;
						that.related =data.related;
